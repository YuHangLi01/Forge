...
def test_all_nodes_count() -> None:
    assert (
        len(WORK_NODES) == 21
    )  # 11 doc-pipeline + 4 ppt-pipeline + 2 lego orchestration + 1 delivery + 2 meta + 1 chat
    assert len(ALL_NODES) == 24  # 21 work + step_router + error_handler + checkpoint_control
