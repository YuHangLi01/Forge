...
def test_plain_text() -> None:
    ...
    assert "text_element_style" not in els[0]


def test_bold() -> None:
    els = _parse("**bold**")
    assert els[0]["text_element_style"]["bold"] is True
    assert els[0]["content"] == "bold"


def test_italic() -> None:
    els = _parse("*italic*")
    assert els[0]["text_element_style"]["italic"] is True


def test_inline_code() -> None:
    els = _parse("`code`")
    assert els[0]["text_element_style"]["inline_code"] is True
    assert els[0]["content"] == "code"


def test_link() -> None:
    els = _parse("[text](https://example.com)")
    assert els[0]["content"] == "text"
    assert els[0]["text_element_style"]["link"]["url"] == "https://example.com"


def test_bold_italic_combined() -> None:
    els = _parse("**bold *italic* end**")
    texts = [e["content"] for e in els]
    joined = "".join(texts)
